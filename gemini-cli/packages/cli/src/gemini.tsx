...
export async function main() {
  ...
  // Skip sandbox/relaunch entirely in HTTP server mode
  if (!process.env['SANDBOX'] && !argv.httpServer) {
  ...
  // may have side effects.
  {
    ...
    // --- HTTP Server mode ---
    // Must be checked before interactive/non-interactive branching.
    if (argv.httpServer) {
      const fs = await import('node:fs');
      const httpLog = (msg: string) => fs.writeSync(2, `[gemini-http] ${msg}\n`);

      httpLog('Starting Gemini CLI HTTP Server...');
      httpLog('Initializing config...');
      await config.initialize();

      httpLog('Authenticating...');
      const authType = await validateNonInteractiveAuth(
        settings.merged.security.auth.selectedType,
        settings.merged.security.auth.useExternal,
        config,
      ...
      await runHttpServer(config, {
        port: argv.httpPort ?? 9000,
        host: argv.httpHost ?? '0.0.0.0',
      });
      await runExitCleanup();
      process.exit(ExitCodes.SUCCESS);
    }
