    ...
    .command('$0 [query..]', 'Launch Gemini CLI', (yargsInstance) =>
      yargsInstance
        ...
        .option('http-server', {
          type: 'boolean',
          description:
            'Start an OpenAI-compatible HTTP server instead of the CLI interface.',
          default: false,
        })
        .option('http-port', {
          type: 'number',
          description:
            'Port for the HTTP server (only used with --http-server).',
          default: 9000,
        })
        .option('http-host', {
          type: 'string',
          description:
            'Host to bind the HTTP server to (only used with --http-server).',
          default: '0.0.0.0',
        }),
    )
